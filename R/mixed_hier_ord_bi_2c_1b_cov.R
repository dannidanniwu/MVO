...
s_generate <- function(sigma_beta_0=0.2, sigma_beta_1=0.1,
  ...
  def <- defData(def, varname = "y_1_hat", 
                 formula = "0.35*cov_1 - 0.4*cov_2 + 0.15*cov_3+
...
mvo_model <- function(iter, generated_data,mod){
  ...
  res <- data.table(beta_0,beta_cov1,beta_cov2,beta_cov3,beta_trt,beta_inter_cov1,
                    ...
                    tau_1,tau_2,tau_3,Phi_1,Phi_2,Phi_3,beta_star_trt,
...
res <- rbindlist(res)
save(res, file = "/gpfs/data/troxellab/danniw/data/mixed_hier_ord_bi_2c_1b_cov.rda")
...
####--plot---#####
#bayes_result <- bayes_result[bayes_result$div <=100,]
# bayes_result <- res[res$div <=100,]
# dim(bayes_result)#110
# #the ordinal outcome
...
#   labs(title="MVO: Posterior mean of paramters (n=400)",
