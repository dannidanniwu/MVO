...
mod <- cmdstan_model("/gpfs/data/troxellab/danniw/r/conti_hier_5cov.stan");


#mod <- cmdstan_model("./conti_hier_5cov.stan");
s_generate <- function(sigma_beta_0=0.2, sigma_beta_1=0.1,
  ...
  def <- defData(def, varname = "y_1", 
                 formula = "-0.3 + 0.35*cov_1 - 0.4*cov_2 + 0.15*cov_3 + 0.2*cov_4 - 0.21*cov_5 +
...
mvo_model <- function(iter, generated_data,mod){
 # set_cmdstan_path(path = "/gpfs/share/apps/cmdstan/2.25.0")
  ...
  y <- model.matrix( ~ y_1+y_2+y_3+y_4, data = generated_data)[,-1]
  ...
  beta_star_4 <- draws_dt[,c(grep("^beta_star.4",colnames(draws_dt),value=TRUE))]
  beta_star_5 <- draws_dt[,c(grep("^beta_star.5",colnames(draws_dt),value=TRUE))]
  beta_star_6 <- draws_dt[,c(grep("^beta_star.6",colnames(draws_dt),value=TRUE))]
  ...
  res <- data.table(beta_0,beta_cov1,beta_cov2,beta_cov3,beta_cov4,beta_cov5,
                    ...
                    tau_1,beta_star_trt,
                    beta_star_2,beta_star_3, 
                    beta_star_4,beta_star_5, 
                    beta_star_6,
                    div,tree_hit)
...
bayes_result <- rbindlist(lapply(1, function(x) s_train(x,sigma_beta_0=0.2, sigma_beta_1=0.1,
                                                        sigma_beta_2 = 0.1,sigma_beta_3 = 0.09,
                                                        sigma_beta_4 = 0.12,sigma_beta_5 = 0.09,
                                                        n_train = 400, mod=mod)))
...
res <- rbindlist(res)
save(res, file = "/gpfs/data/troxellab/danniw/data/hier_conti_3c_2b_cov.rda")
...
gener_y4 <- data.frame(iter=1,
                       ...
                       beta_inter_cov4=0.07,
                       beta_inter_cov5=-0.04,
                       beta_star_trt=0.395,
                       beta_star_2=0.195,
                       beta_star_3=-0.105,
                       beta_star_4=0.105,
...
M_MVO <- subset(bayes_result,select=c(iter,beta_0,beta_cov1,beta_cov2,beta_cov3,
                                      ...
                                      beta_star_trt,
                                      beta_star_2,
                                      beta_star_3,beta_star_4,beta_star_5,beta_star_6))
M_MVO$Md <- "Estimated"
...
ggplot(M_data, aes(x=variable, y=value,fill=Md)) +
  ...
  labs(title="MVO: Posterior mean of paramters (n=400,4 continuous outcomes)",
