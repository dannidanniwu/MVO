...
mod <- cmdstan_model("/gpfs/data/troxellab/danniw/r/mixed_onelayer_ord_binary.stan");
univt_mod <- cmdstan_model("/gpfs/data/troxellab/danniw/r/ord_3c_2b_cov.stan");

#mod <- cmdstan_model("./mixed_onelayer_ord_binary.stan");
...
res <- rbindlist(res)
save(res, file = "/gpfs/data/troxellab/danniw/data/mixed_one_layerhier_gamma1.rda")
...
ggplot(M_data, aes(x=variable, y=value,fill=Md)) +
  ...
  labs(title="Posterior mean of paramters (no individual beta's layer)",
...
ggplot(M_data, aes(x=variable, y=value,fill=Md)) +
  geom_boxplot(width=0.25,position = position_dodge(width = 0.5))+ theme_minimal()+
  labs(title="Posterior SD of paramters (no individual beta's layer)",
...
ggplot(m_data, aes(x=variable, y=value)) +
  geom_boxplot()+
  labs(title="PCD based on simualtions with low div (no individual beta's layer)",
       y = "PCD", x= "model")+
  labs(fill= "Model")
...
ggplot(PCD, aes(y=diff)) +
  geom_boxplot()+
  labs(title="PCD difference based on simualtions with low div (no individual beta's layer)",
       y = "PCD difference")
