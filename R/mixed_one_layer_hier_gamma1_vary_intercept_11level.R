#hierarchical model just with one layer
#change beta_0 to be varied across binary outcomes
#stan code: all binary outcomes have same intercept -> outcome specific intercept
#beta_0 for binary model prior~ N(0,10) change to t(3,0,8)
library(simstudy)
library(data.table)
library(cmdstanr)
library(posterior)
...
#mod <- cmdstan_model("./mixed_twolayer_hier_varyintercept.stan");
...
mvo_model <- function(generated_data,mod){
  ...
  beta_0_b1 <- draws_dt$beta_0.1
  beta_0_b2 <- draws_dt$beta_0.2
  beta_0_b3 <- draws_dt$beta_0.3
  ...
  res <- data.table(beta_0_b1,beta_0_b2,beta_0_b3,beta_cov1,beta_cov2,beta_cov3,beta_cov4,beta_cov5,
...
s_train <- function(sigma_beta_0=1^2, sigma_beta_1=1^2,
  ...
  p_5_g1 <- mean(generated_data$y_ord %in% 5)
  p_6_g1 <- mean(generated_data$y_ord %in% 6)
  p_7_g1 <- mean(generated_data$y_ord %in% 7)
  p_8_g1 <- mean(generated_data$y_ord %in% 8)
  p_9_g1 <- mean(generated_data$y_ord %in% 9)
  p_10_g1 <- mean(generated_data$y_ord %in% 10)
  p_11_g1 <- mean(generated_data$y_ord %in% 11)
