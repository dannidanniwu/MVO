...
s_test_generate <- function(n_test = 2000) {
  ...
  defS <-  defCondition(defS,
                        condition = "(y_trt1 == y_trt0) & (y_trt0 == 1)",
                        formula = "2", 
                        dist = "nonrandom")
  
  defS <-  defCondition(defS,
                        condition = "(y_trt1 == y_trt0) & (y_trt0 == 0)",
                        formula = "3", 
                        dist = "nonrandom")
  #---Generate data---#
  dd <- genData(n_test, def)
  dd <- addCondition(defS, dd, newvar = "y_diff")
  #check
  # dd[,
  #           .(trt1 = max(y_trt1),trt0= max(y_trt0)),
  #           keyby = .(y_diff)]
  return(dd)
}
