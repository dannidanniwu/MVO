...
bayes_single_rep <- function(iter, sigma_beta_0=0.2, sigma_beta_1=0.1,
  ...
  return(data.table(iter, mvo_PCD,
                    ...
                    t(univt_coef_sd),train_coef$prop))
...
res <- rbindlist(res)
save(res, file = "/gpfs/data/troxellab/danniw/data/mixed_hier_gamma1.rda")
...
ggplot(M_data, aes(x=variable, y=value,fill=Md)) +
  ...
  labs(title="Posterior mean of paramters (prior of sigma_j ~ exp(mean=0.01))",
       y = "Posterior mean")+facet_wrap(~variable,scale=c("free"), labeller = label_parsed)+
  theme(strip.text.x = element_blank())+labs(fill="CLASS")

ggplot(M_data, aes(x=variable, y=value,fill=Md)) +
  geom_boxplot(width=0.25,position = position_dodge(width = 0.5))+ theme_minimal()+
  labs(title="Posterior mean of paramters (prior of sigma_j ~ exp(mean=0.1))",
       y = "Posterior mean")+facet_wrap(~variable,scale=c("free"), labeller = label_parsed)+
  theme(strip.text.x = element_blank())+labs(fill="CLASS")

ggplot(M_data, aes(x=variable, y=value,fill=Md)) +
  geom_boxplot(width=0.25,position = position_dodge(width = 0.5))+ theme_minimal()+
  labs(title="Posterior mean of paramters (prior of sigma_j ~ exp(mean=1))",
       y = "Posterior mean")+facet_wrap(~variable,scale=c("free"), labeller = label_parsed)+
  theme(strip.text.x = element_blank())+labs(fill="CLASS")
...
ggplot(M_data, aes(x=variable, y=value,fill=Md)) +
  geom_boxplot(width=0.25,position = position_dodge(width = 0.5))+ theme_minimal()+
  labs(title="Posterior SD of paramters (prior of sigma_j ~ exp(mean=0.01))",
       y = "Posterior SD")+facet_wrap(~variable,scale=c("free"), labeller = label_parsed)+
  theme(strip.text.x = element_blank())+labs(fill="CLASS")

ggplot(M_data, aes(x=variable, y=value,fill=Md)) +
  geom_boxplot(width=0.25,position = position_dodge(width = 0.5))+ theme_minimal()+
  labs(title="Posterior SD of paramters (prior of sigma_j ~ exp(mean=0.1))",
       y = "Posterior SD")+facet_wrap(~variable,scale=c("free"), labeller = label_parsed)+
  theme(strip.text.x = element_blank())+labs(fill="CLASS")

ggplot(M_data, aes(x=variable, y=value,fill=Md)) +
  geom_boxplot(width=0.25,position = position_dodge(width = 0.5))+ theme_minimal()+
  labs(title="Posterior SD of paramters (prior of sigma_j ~ exp(mean=1))",
       y = "Posterior SD")+facet_wrap(~variable,scale=c("free"), labeller = label_parsed)+
  theme(strip.text.x = element_blank())+labs(fill="CLASS")
...
ggplot(m_data, aes(x=variable, y=value)) +
  geom_boxplot()+
  labs(title="PCD based on simualtions with low div (prior of sigma_j ~ exp(mean=0.01)",
       y = "PCD", x= "model")+
  labs(fill= "Model")

ggplot(m_data, aes(x=variable, y=value)) +
  geom_boxplot()+
  labs(title="PCD based on simualtions with low div (prior of sigma_j ~ exp(mean=0.1)",
       y = "PCD", x= "model")+
  labs(fill= "Model")

ggplot(m_data, aes(x=variable, y=value)) +
  geom_boxplot()+
  labs(title="PCD based on simualtions with low div (prior of sigma_j ~ exp(mean=1)",
       y = "PCD", x= "model")+
  labs(fill= "Model")
...
ggplot(PCD, aes(y=diff)) +
  geom_boxplot()+
  labs(title="PCD difference based on simualtions with low div (prior of sigma_j ~ exp(mean=0.01)",
       y = "PCD difference")

ggplot(PCD, aes(y=diff)) +
  geom_boxplot()+
  labs(title="PCD difference based on simualtions with low div (prior of sigma_j ~ exp(mean=0.1)",
       y = "PCD difference")

ggplot(PCD, aes(y=diff)) +
  geom_boxplot()+
  labs(title="PCD difference based on simualtions with low div (prior of sigma_j ~ exp(mean=1)",
       y = "PCD difference")
